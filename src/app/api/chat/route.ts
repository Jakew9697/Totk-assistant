...
function formatGeneralResponse(text: string): string {
  ...
  paragraphs.forEach(paragraph => {
    ...
    if (
      /best|important|recommended|ideal|powerful|effective|strongest|key|notable|tip|hint/i.test(paragraph) &&
...
export async function POST(request: Request) {
  try {
    ...
    const zeldaResponse: Message = {
      role: 'assistant',
      content: formatResponseForReadability(completion.choices[0].message.content ?? '') ||
